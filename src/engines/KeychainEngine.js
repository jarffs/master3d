...
export class KeychainEngine extends BaseEngine {
  ...
  async generateSvgFromTextCanvas(text, family, letterSpacing, lineSpacing) {
    ...
    // Noto Emoji garante emojis monocrom├íticos, vetoriz├íveis junto com o texto
  ...
  buildKeychain(params, svgShapes, itemIndex, lineCount = 1) {
    ...
    // A largura escolhida ├⌐ a da placa final, ent├úo o texto desconta as margens
    ...
    // 1. Criar a base s├│lida (Backing Plate)
    ...
    // Dilata e erode de volta para unir linhas e emojis num corpo ├║nico, preservando o contorno
    ...
    // Apenas contornos externos: a base fica sempre s├│lida, sem os vazios das letras
    fullBasePaths
    ...
    if (fullBasePaths.length > 0) {
      ...
      // Converter ├óngulo para radianos (90 graus = topo no canvas Y para baixo, mas no three Y ├⌐ para cima, vamos ajustar)
      ...
      // O raio interno ├⌐ ringRadius - ringThickness. 
      // Deixamos um extra de 0.2mm de margem de seguran├ºa para garantir que o furo fica limpo.
